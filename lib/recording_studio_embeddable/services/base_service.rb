  ...
  module Services
    ...
    class BaseService
      ...
      def run_before_hooks
        ...
        RecordingStudioEmbeddable::Hooks.run(:before_service, self.class, service_args)
      end
      ...
      def run_after_hooks(result)
        ...
        RecordingStudioEmbeddable::Hooks.run(:after_service, self.class, result)
      end
      ...
      def run_with_around_hooks
        if hooks_enabled? && RecordingStudioEmbeddable.configuration.hooks.registered?(:around_service)
          RecordingStudioEmbeddable::Hooks.run_around(:around_service, self) { perform }
        else
          perform
        end
      end

      # Check if hooks are enabled (RecordingStudioEmbeddable is loaded and configured)
      #
      # @return [Boolean]
      def hooks_enabled?
        defined?(RecordingStudioEmbeddable) &&
          RecordingStudioEmbeddable.respond_to?(:configuration) &&
          RecordingStudioEmbeddable.configuration.respond_to?(:hooks)
      end
